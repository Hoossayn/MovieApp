...
class GetMovieDetailUseCase @Inject constructor(private val movieRepository: MovieRepository) {
    ...
    private val result = PublishSubject.create<Resource<Movie>>()
    private val mCompositeDisposable = CompositeDisposable()

    operator fun invoke(movieId: Int): Flowable<Resource<Movie>> {
        val apiResponse = movieRepository.getMovieById(movieId)
        ...
        val response =  apiResponse.subscribeOn(Schedulers.io())
            ...
            .subscribe { response ->
                try {
                    result.onNext(Resource.success(response.toMovie()))
